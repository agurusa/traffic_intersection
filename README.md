...
- The controller does not keep track of how long each car has been in the queue. Therefore, if traffic is very backed up in each lane, and it takes multiple cycles for a single car to make it to the front of the line, it will take longer than the estimated 4.5 minutes for the car to go through the intersection. Ideally, we could average how long it took for a car to go through the intersection, and average how many cars arrive in the intersection per second (both estimated here as 4 cars per second); this would help us optimize based on traffic conditions (like rush hour). However, here we assume that cars are somewhat evenly distributed between lanes, and that they leave and arrive at the same pace.
